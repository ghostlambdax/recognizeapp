  ...
  def json_for_errors
    ...
    obj.errors.each do |attr, msg|
      ...
      #errors on nested attributes show up in the form of `association.attribute`
        ...
        association_name, nested_attribute = attr.to_s.split(".")
        association = obj.send(association_name)
        reflection = obj.class.reflect_on_association(association_name.to_sym)
        association_klass = reflection.class_name.classify.constantize

        if association.respond_to?(:each)
          # Umm, this might be flaky
          # assumes ordering of the keys in the params is consistent with the ordering in the association...
          # Will return something like ["0", "1491115657475", "1491115658670"]
          association_params = controller.params[obj.class.to_s.demodulize.to_s.underscore]["#{association_name}_attributes"]
          keys = association_params.reject{|k,v| v.has_key?("_destroy") && v["_destroy"] == "1"}.keys
          association.each_with_index do |association_instance, index|
            next if association_instance.errors.blank?

            index_or_id = keys[index]#association_instance.id || index
            association_instance.errors.each do |association_attr, association_msg|
              next unless association_attr.to_sym == nested_attribute.to_sym
              identifier = "#{dom_class(obj)}_#{association_name}_attributes_#{index_or_id}_#{association_attr}"
              h[identifier] ||= []
              if association_msg.starts_with?("^")
                h[identifier] << association_msg.gsub(/^\^/,'')
              else
                h[identifier] << "#{association_klass.human_attribute_name(association_attr)} #{msg}"
              end
              # In case of "variants.face_value" has two errors on same field
              # outloop will be called twice, which causes loop over association twice
              # which causes double error messages
              # This is a bit of a hack that fixes that
              h[identifier].uniq!             
            end
          end
        else
    ...
    error_response = {'type' => 'error', 'errors' => h, 'id' => obj.try(:id)}
      ...
      if o.class.respond_to?(:attributes_for_json)
        h[o.class.to_s.underscore] = o.class.attributes_for_json.inject({}){|hash, a|
          ...
          if attribute_value.kind_of?(CarrierWave::Uploader::Base) # hack!
            hash[a] = attribute_value.as_json
