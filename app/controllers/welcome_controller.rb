class WelcomeController < ApplicationController
  before_action :send_to_subscription, only: [:show], if: ->{params.key?(:upgrade) && @company.subscription.present?}
  ...
  def show
    ...
    @youtube_id = current_user.subscribed_account? ? "l9k_CSBHPNY" : "pss19g3ISII"

    if current_user.company.customizations.present? && current_user.company.customizations.youtube_id.present?
