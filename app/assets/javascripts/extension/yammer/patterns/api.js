...
(function() {
  ...
  Recognize.patterns.Api = Api;

  //TODO - make this dynamic somehow(local, staging, production)

  function Api() {
    BASE_ENDPOINT = Recognize.host+'/api/v1';
    Recognize.patterns.Api.endPoint = BASE_ENDPOINT;
  };
  ...
  Api.prototype.unauthenticated = function(data) {
    if (Recognize.accessToken) {
      Recognize.accessToken.clear();
    }
    Recognize.utils.showAuthorizationHeader();
