class Report::User
  ...
  def initialize(user, from=50.years.ago, to=Time.current, opts={})
    @user = user
    @from = from
    @to = to
    @opts = opts
  ...
  def redeemed_points
    @redeemed_points ||= calculate_redeemed_points!
  end
  ...
  def sent_recognition_count
    @sent_recognition_count ||= Recognition.approved.where(sender_id: user.id)
  ...
  def received_recognition_count
    @received_recognition_count ||= RecognitionRecipient.of_approved_recognitions.joins(:recognition)
