module YammerUser
  ...
  def refresh_cached_yammer_groups!(client = self.yammer_client)
    ...
    Rails.logger.debug "Caught exception refreshing cached yammer groups: #{e.message}"
  end

  # Fetch all groups for the user's network.
  ...
  #   Note: Pagination not required, as per docs
  def yammer_groups_for_user(client = self.yammer_client)
    return [] unless self.yammer_id.present? and client.authenticated?
    client.groups_for_user(self.yammer_id)
  rescue YammerClient::Unauthorized => e
    client.handle_unauthorized(e, self)
  end

  def postable_yammer_groups
    yammer_groups_for_user.reject { |group| group.restricted_posting == true }
  end

  #
  # 'All Company' is a group in yammer which has all users in the network. Among its other significances, one is for
  #   endpoint `POST - /api/v1/messages`. If 'All Company' group does not have a numeric 'feed_id'(or 'group_id') then
  #   a post without direct_to_user_ids, replied_to_id or group_id will default to All Company group.
  #
  # For more:
  #   - https://developer.yammer.com/docs/messages-json-post
  #   - https://support.microsoft.com/en-us/office/restrict-all-company-posts-in-yammer-3219d2ae-db15-4c9f-9dd2-28559ae39a97?ui=en-us&rs=en-us&ad=us
  #   - https://docs.microsoft.com/en-us/yammer/manage-yammer-groups/yammer-all-company-yammer-community
  #
  def yammer_all_company_group
    groups_for_company = ProviderGroupService.yammer_groups_for_company(self)
    groups_for_company.detect { |group| group.name == "allcompany" }
  ...
  def refresh_yammer_client!
    ...
    Rails.logger.debug "Caught exception refreshing yammer client: #{e.message}"
  end
  ...
  def load_yammer_client
    # reload user to make sure associations aren't put into client/cache
    yc = YammerClient.new(self.yammer_token, User.find(self.id))
