class PointHistory < ApplicationRecord
  ...
  validates :start_date, uniqueness: {scope: [:owner_id, :owner_type], case_sensitive: true}
  validates :end_date, uniqueness: {scope: [:owner_id, :owner_type], case_sensitive: true}
