...
class Reminder < ApplicationRecord
  ...
  class Process
    ...
    # Reminders are always run every day so that we can figure out in ruby
    # which day of the week, or other app logic should determine what to run
    # Such as: Run the monthly's only on the first monday of the month. 
    def self.daily
      ...
      # end
      time = Time.current
      Rails.logger.info "#{ Time.current.to_formatted_s(:db)} - Beginning Reminder#daily"
      results = []
      TimezoneEnforcer.run(hour_to_run: 6, company_scope: Company.program_enabled) do |company_ids, time|
        results << run_process(time, company_ids)  if company_ids.size > 0
      end
      Rails.logger.info " ------------- #{Time.current.to_formatted_s(:db)} - Completed Reminder#daily"
      return results
    end

    def self.run_process(time, company_ids)
      # gather results here is mainly for simulation
      # because in real life, we'd get back the delayed job object
      process = Process.new
      # if time.monday? and time.day.between?(1, 7)
      #   results = process.run!(as: :monthly, company_ids: company_ids)
      # elsif time.monday?
      #   results = process.run!(as: :weekly, company_ids: company_ids)
      # elsif time.wday.between?(1,5) # only run during the week
        results = process.run!(as: :daily, company_ids: company_ids)
      # end
      return results
    end
    ...
    def run!(opts={})
      ...
      # else daily
      when :daily
        results << [:daily_blast, Reminder.send_daily_blast(company_ids: opts[:company_ids]), Time.zone.name]
