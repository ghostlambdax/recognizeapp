class InboundEmail < ApplicationRecord
  ...
  def release!
    return if [PROCESSED, ERRORED].include?(status.to_i)

    if recipient_emails.blank?
      handle_no_recipients!
    else
      recognition = sender.recognize!(recipient_emails, badge, message, post_to_yammer_wall: true, post_to_yammer_group_id: post_to_yammer_group_id, from_inbound_email_id: self.id)
      InboundEmailNotifier.confirmation(recognition).deliver_now
    end

    return recognition
  rescue Exception => e
    exception_data =  {inbound_email_id: self.id, sender_email: self.sender_email, recipients: recipient_emails}
    exception_data[:recognition_errors] = recognition.errors.full_messages.to_sentence if recognition.errors.present?
    ExceptionNotifier.notify_exception(e, {data: exception_data})
  ensure
    self.update_column(:status, ERRORED)
  end

  def post_to_yammer_group_id
    sender.company.post_to_yammer_group_id
  end
  ...
  def handle_no_recipients!
    InboundEmailNotifier.missing_recipients(self).deliver_now
  end

  def filter_legit_emails(set)
    set
      .flatten
      .select{|item| item && item.match(Constants::EMAIL_REGEX)}
  ...
  def process!
    if sender.present? && sender.active?
      ...
      if sender
        UserNotifier.verification_email(sender).deliver_now
      else
