...
class CompanyReporter
  ...
  class UserLite < Struct.new(:id, :email, :first_name, :last_name, :display_name, :total_points, :sent_recognitions_count, :received_recognitions_count, :slug, :network)
    include HashIdConcern    
    def full_name
      return User.safe_full_name(email, first_name, last_name, display_name)
    end
  end
  ...
  def load_users
    ...
    time = Benchmark.realtime{
      users = @company.users.pluck(:id, :email, :first_name, :last_name, :display_name, :total_points, :sent_recognitions_count, :received_recognitions_count, :slug, :network).collect{|u| UserLite.new(*u)}
    } 
  ...
  def most_validated_employees(limit=10)
    ...
    recognitions_set.each do |r|
      r.user_recipients.each do |user|
        ...
        h[user.id] ||= RecognizeOpenStruct.new(recipient: user, approval_count: 0)
